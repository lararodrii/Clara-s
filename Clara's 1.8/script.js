document.addEventListener('DOMContentLoaded', function () {
    ...
    // const durationInputs = document.getElementsByName('duration'); // REMOVED
    ...
    function calculateTotal() {
        ...
        if (guests <= 30) {
            basePricePerPerson = PRICES.base3h; // Regra: Até 30 pessoas = 3 Horas
        } else {
            basePricePerPerson = PRICES.base4h; // Regra: Mais de 30 pessoas = 4 Horas
        }
    ...
    if (calendarDays && monthYear) {
        ...
        function openBookingModal(date) {
            ...
            if (modalGuestsInput) {
                modalGuestsInput.value = globalGuests;
            }
        }
    ...
    // Freight Logic
    const eventDistanceInput = document.getElementById('event-distance');
    const freightValueSpan = document.getElementById('freight-value');
    const btnCheckDistance = document.getElementById('btn-check-distance');
    let freightCost = 0;

    if (eventDistanceInput) {
        eventDistanceInput.addEventListener('input', () => {
            const distance = parseFloat(eventDistanceInput.value) || 0;
            freightCost = distance * 2.00;
            if (freightValueSpan) {
                freightValueSpan.textContent = freightCost.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
            }
        });
    }

    if (btnCheckDistance) {
        btnCheckDistance.addEventListener('click', (e) => {
            e.preventDefault();
            const destination = document.getElementById('event-location').value;
            if (destination) {
                const origin = "Arniqueiras Residencial Diamante, Brasília";
                const url = `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(origin)}&destination=${encodeURIComponent(destination)}`;
                window.open(url, '_blank');
            } else {
                alert("Por favor, preencha o local do evento primeiro.");
            }
        });
    }
    ...
    if (reviewBookingBtn) {
        reviewBookingBtn.addEventListener('click', () => {
            ...
            // const duration = document.getElementById('event-duration').value; // REMOVED
            const time = document.getElementById('event-time').value;
            const endTime = document.getElementById('event-end-time').value;
            const guests = document.getElementById('modal-guests').value;

            if (!name || !location || !time || !endTime || !guests) {
            ...
            if (selectedDate.toDateString() === now.toDateString()) {
                ...
                if (selectedStartMinutes < currentMinutes) {
                    availabilityMessage.textContent = "Para hoje, o horário de início deve ser maior que o horário atual.";
                    availabilityMessage.style.color = "red";
                    return;
                }
            }
            ...
            // Handle overnight events (e.g. ends at 02:00 next day)
            if (endTotalMinutes < startTotalMinutes) {
                endTotalMinutes += 24 * 60;
            }

            if (endTotalMinutes <= startTotalMinutes) {
                availabilityMessage.textContent = "O horário de término deve ser após o horário de início.";
                availabilityMessage.style.color = "red";
                return;
            }
            ...
            summaryContent.innerHTML = `
                ...
                <p><strong>Distância:</strong> ${eventDistanceInput.value} Km (Frete: ${freightCost.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })})</p>
                <p><strong>Serviços:</strong> ${servicesText}</p>
                <p><strong>Orçamento Estimado:</strong> ${globalEstimatedTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
            `;
    ...
    function calculateEndTime() {
        ...
        if (guests > 30) {
            duration = 4; // 4 hours if > 30 guests
        }
        ...
        // Handle day rollover (e.g., 22:00 + 4h = 26:00 -> 02:00)
        if (endHours >= 24) {
        ...
        eventEndTimeInput.value = `${formattedHours}:${formattedMinutes}`;
    }
    ...
    if (modalGuestsInput) modalGuestsInput.addEventListener('input', calculateEndTime);
            ...
            fetch(scriptURL, {
                ...
                .then(data => {
                    if (data.status === 'success') {
                        ...
                        const message = `Olá, gostaria de confirmar meu agendamento:%0A%0A*Nome:* ${name}%0A*Data:* ${selectedDate.toLocaleDateString('pt-BR')}%0A*Convidados:* ${guests}%0A*Horário:* ${time} às ${endTime}%0A*Local:* ${location}%0A*Distância:* ${distance} Km (Frete: ${freightCost.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })})%0A*Serviços:* ${servicesText}%0A*Orçamento Estimado:* ${globalEstimatedTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`;
